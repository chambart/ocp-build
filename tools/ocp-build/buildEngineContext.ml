...
let get_file_uid filename =
  ...
  if st.Unix.st_ino = 0 &&
    match Win32.os_type with
      Win32.UNIX -> false
    | Win32.CYGWIN | Win32.WINDOWS -> true
    then
    let ft = Win32Fileinfo.getFileInformationByName filename in
    st, (ft.Win32Fileinfo.dwVolumeSerialNumber,
	 ft.Win32Fileinfo.nFileIndex)
  else
    st, (st.Unix.st_dev, Int64.of_int st.Unix.st_ino)
