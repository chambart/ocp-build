...
and structure_item sitem st : t list = match sitem.str_desc with
  ...
  | Tstr_primitive ( id, _ , _) -> Primitive id :: st
  | Tstr_type id_decls -> List.map (fun ( id, _ ,_) -> Type id) id_decls @ st
  | Tstr_exception ( id, _, _)
  | Tstr_exn_rebind ( id, _, _, _) -> Exception id :: st
  | Tstr_module (id, _, mexp) -> Module (id, module_expr mexp) :: st
  | Tstr_recmodule id_mty_mexps -> List.map (fun (id, _, _, mexp) -> Module (id, module_expr mexp)) id_mty_mexps @ st
  | Tstr_modtype (id, _, mty) -> Module_type (id, module_type mty) :: st
  | Tstr_open _ -> st
  ...
  | Tstr_class_type id_cinfoss ->
      List.map (fun (id, _, cinfos) -> Class_type [id; cinfos.ci_id_class; cinfos.ci_id_class_type; cinfos.ci_id_object; cinfos.ci_id_typesharp]) id_cinfoss @ st
...
and signature_item sitem st : t list = match sitem.sig_desc with
  | Tsig_value (id, _, { val_val = { val_kind = Val_prim _; _ }; _} ) -> Primitive (id) :: st
  | Tsig_value (id, _, _) -> Value (id) :: st
  | Tsig_type id_decls -> List.map (fun (id, _, _) -> Type (id) ) id_decls @ st
  | Tsig_exception (id, _, _) -> Exception (id) :: st
  | Tsig_module (id, _, mty) -> Module (id, module_type mty) :: st
  | Tsig_recmodule id_mtys -> List.map (fun (id, _, mty) -> Module (id, module_type mty)) id_mtys @ st
  | Tsig_modtype (id, _, mtyd) -> Module_type (id, modtype_declaration mtyd) :: st
  | Tsig_open _ -> st
...
and module_expr mexp : module_ = match mexp.mod_desc with
  | Tmod_ident (p, _) -> Path p
  | Tmod_structure str -> Structure (structure str)
  | Tmod_functor (id, _, _, mexp) -> Functor (id, module_expr mexp)
...
and module_type mty : module_ = match mty.mty_desc with
  | Tmty_ident (p, _) -> Path p
  | Tmty_signature sg -> Structure (signature sg)
  | Tmty_functor (id, _, _mty1, mty2) -> Functor (id, module_type mty2)
...
module Format = struct
  ...
  and structure_item ppf sitem = match sitem.str_desc with
    ...
    | Tstr_primitive (id, _, _) -> fprintf ppf "prim %a" Ident.format (id)
    | Tstr_type id_decls -> List.iter (fun (id, _, _) -> fprintf ppf "type %a" Ident.format (id)) id_decls
    | Tstr_exception (id, _, _)
    | Tstr_exn_rebind (id, _, _, _) -> fprintf ppf "exception %a" Ident.format (id)
    | Tstr_module (id, _, mexp) -> fprintf ppf "@[<2>module %a =@ @[%a@]@]" Ident.format (id) module_expr mexp
    | Tstr_recmodule id_mty_mexps ->
        fprintf ppf "@[<2>module rec @[%a@]@]"
          (Format.list " and " (fun ppf (id, _, _, mexp) ->
            fprintf ppf "@[<2> %a = @ @[%a@]@]" Ident.format (id) module_expr mexp))
          id_mty_mexps
    | Tstr_modtype (id, _, mty) -> fprintf ppf "@[<2>modtype %a = @[%a@]@]" Ident.format (id) module_type mty
    | Tstr_open _ -> ()
    | Tstr_class cinfoss ->
        List.iter (fun (cinfos, _, _) -> fprintf ppf "@[<2> class @[%a@]@]" class_infos cinfos) cinfoss
    | Tstr_class_type id_cinfoss ->
        List.iter (fun (id, _, cinfos) ->
  ...
  and signature_item ppf sitem = match sitem.sig_desc with
    | Tsig_value (id, _, { val_val = { val_kind = Val_prim _; _ }; _} ) -> fprintf ppf "prim %a" Ident.format (id)
    | Tsig_value (id, _, _) -> fprintf ppf "val %a" Ident.format (id)
    | Tsig_type id_decls ->
        List.iter (fun (id, _, _) -> fprintf ppf "type %a" Ident.format (id)) id_decls
    | Tsig_exception (id, _, _) -> fprintf ppf "exception %a" Ident.format (id)
    | Tsig_module (id, _, mty) -> fprintf ppf "@[<2>module %a =@ @[%a@]@]" Ident.format (id) module_type mty
    | Tsig_recmodule id_mtys ->
        fprintf ppf "@[<2>module rec @[%a@]@]"
          (Format.list " and " (fun ppf (id, _, mty) ->
            fprintf ppf "@[<2> %a = @ @[%a@]@]" Ident.format (id) module_type mty))
          id_mtys
    | Tsig_modtype (id, _, mtyd) ->
  ...
  and module_expr ppf mexp = match mexp.mod_desc with
    | Tmod_ident (p, _) -> Path.format ppf p
    | Tmod_structure str -> structure ppf str
    | Tmod_functor (id, _, _, mexp) ->
  ...
  and module_type ppf mty = match mty.mty_desc with
    | Tmty_ident (p, _) -> Path.format ppf p
    | Tmty_signature sg -> signature ppf sg
    | Tmty_functor (id, _, _mty1, mty2) ->
  ...
  let binary_part ppf part = match part with
    | Typedtree.Partial_structure_item _
    | Typedtree.Partial_signature_item _
    | Typedtree.Partial_expression _
    | Typedtree.Partial_module_type _
    | Typedtree.Partial_pattern _
    | Typedtree.Partial_class_expr _
        -> ()
    | Typedtree.Partial_signature sg -> signature ppf sg
    | Typedtree.Partial_structure str -> structure ppf str

  let saved_type ppf = function
    | Packed _ -> ()
    | Typedtree.Implementation str -> structure ppf str
    | Typedtree.Interface sg -> signature ppf sg
    | Typedtree.Partial_implementation parts
    | Typedtree.Partial_interface parts ->
      Array.iter (binary_part ppf) parts
