...
module PARSETREE : sig
  val signature :
    V4000_types.Parsetree.signature -> Parsetree.signature
  val structure :
    V4000_types.Parsetree.structure -> Parsetree.structure
  ...
  open Parsetree
  module T = V4000_types.Parsetree
...
(*
  val reset : unit -> unit
*)
  val t : V4000_types.Ident.t -> Ident.t

end = struct

  module T = V4000_types.Ident
  open Ident
...
module PATH : sig
  ...
  val t : V4000_types.Path.t -> Path.t

end = struct

  module T = V4000_types.Path
  open Path
...
module CMI = struct
  ...
  open V4000_input_ast.AST

module Primitive : sig

  val description : V4000_types.Primitive.description -> Primitive.description

end = struct

    module T = V4000_types.Primitive
    open Primitive
...
module TYPES : sig
    ...
    val signature :
      V4000_types.Types.signature_item list -> Types.signature_item list

    val record_representation :
      V4000_types.Types.record_representation ->
...
module Cmi_format : sig
    ...
    val pers_flags :
      V4000_types.Cmi_format.pers_flags -> Cmi_format.pers_flags
    ;;

  end = struct

    module T = V4000_types.Cmi_format
    open Cmi_format
...
let input_cmi_file ic magic =
  if magic <> V4000_types.cmi_magic_number then
    raise (Error (No_Such_Magic ("cmi", magic)));

  CMI.Ident.reset ();
  CMI.Types.reset ();

  let (cmi_name, cmi_sign) = (input_value ic : string *  V4000_types.Types.signature_item list) in
  let cmi_crcs = (input_value ic : (string * Digest.t) list) in
  let cmi_flags = (input_value ic : V4000_types.Cmi_format.pers_flags list) in
...
let input_cmi ic =
  let (cmi_name, cmi_sign) =
    (input_value ic : string *  V4000_types.Types.signature_item list) in
  let cmi_crcs = (input_value ic : (string * Digest.t) list) in
  let cmi_flags = (input_value ic : V4000_types.Cmi_format.pers_flags list) in
...
let input_ast_intf ic =
  ...
  let intf = (input_value ic : V4000_types.Parsetree.signature) in
  (input_name, intf)

let input_ast_impl ic =
  let input_name = (input_value ic : string) in
  let impl = (input_value ic : V4000_types.Parsetree.structure) in
  (input_name, impl)
